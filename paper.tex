...
Later, \emph{rxjs-spy} can be advised to log events such as subscribe, emit, error, complete and unsubscribe of matching tags directly to the console without touching any \mintinline{TypeScript}{Observable} code at all. It is worth to mention that manual log traces, without additional effort, only show values flowing by. The additional events logged by \emph{rxjs-spy} proofed being invaluable in our own experience multiple times.

The library provides further features to interact with tagged \mintinline{TypeScript}{Observable}s through the debuggers console.
...
\subsection{Rx Vizualizer}

The \emph{Rx Visualizer}\cite{rxviz} is a utility available online in the browser. It is an ``animated playground for Rx Observables''\cite{rxviz} and allows to visualize snippets of RxJS code using marble diagrams.
